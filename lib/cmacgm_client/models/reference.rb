  ...
  class Reference
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CMACGMClient::Reference` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CMACGMClient::Reference`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @reference_value.nil?
        invalid_properties.push('invalid value for "reference_value", reference_value cannot be nil.')
      end

      if @reference_value.to_s.length > 100
        invalid_properties.push('invalid value for "reference_value", the character length must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @reference_value.nil?
      return false if @reference_value.to_s.length > 100
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] reference_value Value to be assigned
    def reference_value=(reference_value)
      if reference_value.nil?
        fail ArgumentError, 'reference_value cannot be nil'
      end

      if reference_value.to_s.length > 100
        fail ArgumentError, 'invalid value for "reference_value", the character length must be smaller than or equal to 100.'
      end
    ...
    def hash
      [reference_type, reference_value].hash
    end
